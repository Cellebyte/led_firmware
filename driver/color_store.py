...
class ColorStore:
    ...
    def validate_value(self, value):
        assert isinstance(value, (rgb.RGB, None)), VALUE_NOT_OF_TYPE(
            self.__class__.__name__, self.name, value, rgb.RGB
        )

    def __setitem__(self, key: int, value: rgb.RGB):
        self.validate_key(key)
        self.validate_value(value)
        self.store.save(self.get_key(key), value.as_dict())

    def __getitem__(self, key: int) -> rgb.RGB:
        self.validate_key(key)
